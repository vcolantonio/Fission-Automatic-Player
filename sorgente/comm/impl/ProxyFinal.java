...
public class ProxyFinal extends Proxy {
    ...
    public ProxyFinal() throws IOException {
        super();
    ...
    public ProxyFinal(Grid grid) throws IOException {
        super(grid);
    ...
    private boolean doActionFor(Message message) throws InterruptedException {
        if(CommonVars.DEBUG) System.out.println(message);

        if (message.type == Message.MessageType.WELCOME) {

            Color color = Color.valueOf(message.tokens.get(0).toUpperCase());
            if(color == Color.WHITE){
                Player player = new PlayerMinMaxVarStrategy(this,
            ...
            this.gameState.setPlayer(color);
        ...
        if (message.type == Message.MessageType.OPPONENT_MOVE) {
            ...
            if(CommonVars.DEBUG) System.out.println(m);
